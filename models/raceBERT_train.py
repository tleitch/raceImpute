...
# args, modify the batch size here to accomodate the vram size
N_EPOCHS = 4
...
# added raceBERT model, pay attention to name_col to be the right one
def get_trainer(model_name, dataset_name):
    ...
    if model_name == "bert":
        ...
        name_col = "first_last"
    elif model_name == "raceBERT":


        tokenizer = AutoTokenizer.from_pretrained("pparasurama/raceBERT", use_fast=True)

        model = RobertaForSequenceClassification.from_pretrained("pparasurama/raceBERT", 
            num_labels=num_labels,
            output_attentions=False,
            output_hidden_states=False,)
        name_col = "first_last"
...
if __name__ == "__main__":
    ...
    # wandb = weights and bias, a training monitoring platform, disabled here as it may creates more confusion in the agency environment
    os.environ["WANDB_DISABLED"] = "true"
    # wandb.init(project="raceBERT")
    # run_name = wandb.run.name
    
    # probably name this something meaningful
    run_name = 'test'
